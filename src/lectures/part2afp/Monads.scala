...
object Monads extends App{
  ...
  object MyLazy {
    def apply[A](a: => A): MyLazy[A] = new MyLazy(a)
  }

  val lazyInstance = MyLazy {
  ...
  val flatMappedInstance = lazyInstance.flatMap(x => MyLazy(10 * x))
  println(flatMappedInstance.use)
  val flatMappedInstance2 = lazyInstance.flatMap(x => MyLazy(10 * x))
  ...
  /*
   ...
   Monads = unit + map + flatten
         Monad[T] {
          ...
          def flatten(m: Monad[Monad[T]]): Monad[T] = m.flatMap(_)
          (have List in mind)
  */
}
